...
|---|---|---|
| 학생 계정 개별 생성 | `admin-account-api` → `admin_create_student_account` | 테스트 서버에 함수가 누락돼 있었다. 동일 요청·응답 계약을 유지하는 테스트 전용 함수를 준비했다. 관리자 세션·현재 계정 상태를 다시 확인하고, 현재 학년도의 등록대기 학생으로만 생성한다. |
...
새 학생 계정 생성 함수의 원격 배포·실제 생성 검증 여부는 최종 배포 기록을 따른다. 위의 ‘존재·계약 일치’는 모든 관리자 작업을 실제 데이터로 수행했다는 의미가 아니다.

## 확인된 연결 수정 사항

1. **학생 계정 생성 API 누락**: `supabase/functions/admin-account-api/index.ts`를 테스트 프로젝트에 배포한다. 별도 외부 패키지가 없고 실제 프로젝트 URL을 확인한다. 성공 후 관리자 배너의 **계정 활성화**에서 코드를 발급한다. 운영본의 약한 세션 검사를 그대로 복사하지 않았다.
2. **새 수업 현황판 식별자**: 수업 ID가 없는 자료의 채점 기록은 `content_`와 하이픈을 제거한 자료 UUID로 저장된다. 자료 목록의 현황판용 식별자도 같은 규칙을 사용해야 한다. 기존 15차시처럼 `lesson_id`가 있는 경우 해당 ID를 그대로 유지한다.
3. **활성화 안내 문구**: 학생 생성·비밀번호 초기화 후 ‘다음 로그인에서 바로 비밀번호 설정’이라는 예전 안내 대신 **계정 활성화에서 코드 발급 → 학생이 코드와 새 비밀번호 입력**을 안내해야 한다.
