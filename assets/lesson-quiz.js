...
(() => {
  ...
  const request=(action,payload)=>window.LessonContent.request(action,payload);
  const record=()=>window.SCIENCE_LESSON_RECORD;
  const identity=()=>window.LessonContent.identity();
  const student=()=>{const user=identity();return user&&user.isAdmin!==true&&String(user.studentSessionToken||'').startsWith('stu_');};
  ...
  function updateControls(){
    ...
    button.disabled=busy||!!beginPromise||!!attempt?.submitted||answered!==questions.length||!student();button.textContent=(busy||beginPromise)?'⏳ 서버 기록 확인 중…':attempt?.submitted?'✅ 제출 완료 · 복습할 답을 골라 보세요':'📤 문제풀이 결과 교사에게 제출';
    if(!student())status(identity()?.isAdmin?'교사용 보기입니다. 실제 풀이와 포인트 지급은 학생 계정으로 확인하세요.':'학생 계정으로 로그인하면 답과 제출 결과를 기록할 수 있습니다.');
    else if(beginPromise)status('기존 풀이 기록과 문항 버전을 확인하고 있습니다…');
    else if(attempt?.submitted)status('제출 완료 · 자유롭게 답을 다시 골라 복습할 수 있습니다.');
    ...
    if(summary){summary.hidden=!attempt?.submitted;if(attempt?.submitted){const result=attempt.result||{};summary.textContent='제출 결과 '+Number(result.score||0)+' / '+Number(result.total||questions.length)+'점 · 제출 지급 기록 '+Number(result.awardedPoints||0)+'P'+(Number.isFinite(Number(result.currentPoints))?' · 현재 잔액 '+Number(result.currentPoints)+'P':'')+(result.duplicate?' · 이미 처리된 제출을 다시 불러왔습니다.':'');}}
  }
  function adopt(data){
  ...
  function reset(){epoch++;lastError='';attempt=null;beginPromise=null;busy=false;reviewChoices={};reviewFeedback={};submitRequest=crypto.randomUUID();for(const box of document.querySelectorAll('.quiz-feedback-box')){box.style.display='none';box.replaceChildren();}updateControls();}
