...
(async () => {
  'use strict';
  const VERSION='20260913-common3';
  ...
  function script(path){return new Promise((resolve,reject)=>{const el=document.createElement('script');el.src=path+'?v='+VERSION;el.onload=resolve;el.onerror=()=>reject(new Error('공통 수업 기능을 불러오지 못했습니다.'));document.body.append(el);});}
  ...
  function quizRender(content){
    ...
    if(typeof window.checkAndApplyStudentAuth==='function')window.checkAndApplyStudentAuth();
  }
  ...
  try{
    ...
    const result=await window.ScienceContentClient.request('get_content',{id});item=result.item;pack=result.content;
    ...
    sharedCSS=await themeResponse.text();document.body.innerHTML=await shellResponse.text();
    ...
    window.addEventListener('science-account-change',()=>{const next=JSON.stringify(window.ScienceContentClient.auth());if(next===owner)return;owner=next;if(sandbox)sandbox.destroy();window.ScienceQuiz?.reset();location.reload();});
